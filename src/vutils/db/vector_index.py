...
class VectorIndex:
    ...
    def from_index_file(dir_path: str) -> "VectorIndex":
        ...
        if not os.path.exists(index_file) or not os.path.exists(properties_file):
            print("No index file found, create a new one.")
            return VectorIndex()
    ...
    def search(self, query_vector: List[float], top_k: int, dis_threshold: float = 0.9) -> List[Any]:
        ...
        for dist, i in zip(distances[0], indices[0]):
            if dist < dis_threshold:  # Smaller distances indicate higher similarity
                ...
                print("[✓] Add valid distance %s to results.", dist)
            else:
                print("[x] Distance %s ≥ threshold %s, ignore this result.", dist, dis_threshold)
        return results
